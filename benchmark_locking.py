...
async def main():
    ...
    for concurrency in concurrency_levels:
        ...
        logger.info(f'  Throughput: {result["throughput"]:.2f} jobs/sec '
...
def generate_chart(pessimistic_results, optimistic_results, output_path):
    ...
    bars1 = ax.bar([i - width/2 for i in x], pess_throughput, width, 
                   label='Pessimistic (SELECT FOR UPDATE)', color='#2196F3', alpha=0.8)
    bars2 = ax.bar([i + width/2 for i in x], opt_throughput, width,
                   label='Optimistic (Version Check)', color='#FF9800', alpha=0.8)
    
    ax.set_xlabel('Concurrency Level', fontsize=12)
    ax.set_ylabel('Throughput (jobs/sec)', fontsize=12)
    ax.set_title('Pessimistic vs Optimistic Locking: Throughput Comparison', fontsize=14)
    ax.set_xticks(x)
