...
2. Execute the wasted work test:
   ...
   ```
   _Expected Outcome_: The client times out, but a row is still written to the PostgreSQL database.
...
2. Execute the deadline fix test:
   ...
   ```
   _Expected Outcome_: The client times out, and NO row is written to the database.
...
2. Run the write skew script:
   ...
   ```
   _Expected Outcome_: The database count will exceed the batch threshold (typically `10`), and the script exits with code `1`.
...
2. Run the write skew script:
   ...
   docker compose exec client python reproduce_write_skew.py --concurrent 30
   ```
   _Expected Outcome_: The script manages transaction serialization properly, counts stay within the threshold, and the script exits with code `0` (Success).
