...
async def main():
    ...
    parser.add_argument('--threshold', type=int, default=int(
        os.environ.get('BATCH_THRESHOLD', '10')),
    ...
    if final_count > args.threshold:
        logger.error(f'WRITE SKEW DETECTED! Count ({final_count}) exceeds threshold ({args.threshold})')
        logger.error(f'This proves the write skew anomaly — {final_count - args.threshold} extra rows!')
        sys.exit(1)
    else:
        logger.info(f'No write skew: count ({final_count}) is within threshold ({args.threshold})')
        sys.exit(0)
